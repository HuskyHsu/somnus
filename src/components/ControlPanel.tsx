...
export const ControlPanel: React.FC<Props> = ({
  ...
  const [cueVol, setCueVol] = useState<number>(() => {
    const saved = localStorage.getItem('somnus_cue_volume');
    return saved !== null ? parseFloat(saved) : 1.0;
  });
  const [noiseType, setNoiseType] = useState<'zen' | 'campfire' | 'ocean'>(() => {
    const saved = localStorage.getItem('somnus_noise_type');
    return saved === 'zen' || saved === 'campfire' || saved === 'ocean' ? saved : 'zen';
  });
  ...
  return (
    ...
    >
      <div
        className={`mb-12 flex flex-col items-center transition-all duration-500 ${isPlaying ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
      >
        <p className='text-white/40 text-sm font-light tracking-[0.2em] mb-2 uppercase'>Somnus</p>
        <p className='text-white/80 text-xl font-light tracking-widest'>4-7-8 Deep Relaxation</p>
      </div>

      <button
        onClick={onToggle}
        className={`px-10 py-4 mb-12 bg-white/5 border border-white/10 rounded-2xl text-white/70 hover:bg-white/10 hover:text-white transition-all tracking-widest font-light cursor-pointer shadow-lg backdrop-blur-md ${!isInteractive ? 'pointer-events-none' : ''}`}
      >
        {isPlaying ? 'Pause' : 'Start'}
      </button>

      <div
        className={`flex flex-col gap-6 w-72 p-6 bg-white/5 border border-white/5 rounded-2xl max-h-[50vh] overflow-y-auto no-scrollbar ${!isInteractive ? 'pointer-events-none' : ''}`}
      >
